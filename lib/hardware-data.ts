...
// --- CPUs MAPPING LAYER ---
export const CPUs: CPU[] = deduplicateById(
  cpusJson.map((c: any) => {
...
// --- GPUs MAPPING LAYER ---
export const GPUs: GPU[] = deduplicateById(
  gpusJson.map((g: any) => {
...
// --- RAMs DEDUPLICATED LIST ---
export const RAMs: RAM[] = deduplicateById(
  ramJson.map((r: any, i: number) => {
    // Append index variable suffix to fallback string to protect key uniqueness boundaries
    const fallbackId = `${r.brand}-${r.speed}-${r.capacity}-${i}`.toLowerCase().replace(/[^a-z0-9]/g, '-')
    return {
      id: r.id || fallbackId,
      name: r.name || `${r.brand || ''} ${r.speed || ''} ${r.capacity || ''}`.trim(),
      brand: r.brand || 'Generic',
      type: r.type || 'DDR5',
      speed: r.speed || 4800,
      capacity: r.capacity || '16GB',
      price: r.price || 0,
    }
  })
)
